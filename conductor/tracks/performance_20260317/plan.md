...
- [ ] Task: Enhance CSS and JS Minification
...
- [ ] Task: Optimize Liquid Template Loops
    - [ ] Identify and optimize heavy Liquid loops (e.g., in `search.liquid` or `archive.liquid`)
    - [ ] Reduce the number of `include` calls where possible
    - [ ] Verify build time reduction
- [ ] Task: Implement Resource Hints
    - [ ] Add `dns-prefetch` and `preconnect` for external resources (Google Fonts, Analytics)
    - [ ] Add `preload` for critical assets (main CSS, key fonts)
    - [ ] Verify impact on First Contentful Paint (FCP)
